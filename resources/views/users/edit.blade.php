...
@section('title', __('ui.user_form.edit_title'))

@section('content_header')
    <h1>{{ __('ui.user_form.edit_title') }}: {{ $user->name }}</h1>
@endsection
    ...
    <div class="card-body">
        <form action="{{ route('users.update', $user) }}" method="POST">
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="email" name="email" class="form-control @error('email') is-invalid @enderror"
                            value="{{ old('email', $user->email) }}" required placeholder="ime.prezime@mup.hr">
                        @error('email')
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <label>{{ __('ui.user_form.password_edit') }} <small class="text-muted">({{ __('ui.user_form.password_hint') }})</small></label>
                        ...
                        <select id="administration_select" class="form-control">
                            ...
                            @foreach($administrations as $admin)
                                <option value="{{ $admin->id }}"
                                    {{ (old('administration_id', $user->station?->police_administration_id) == $admin->id) ? 'selected' : '' }}>
                                    {{ $admin->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <a href="{{ route('users.show', $user) }}" class="btn btn-secondary">{{ __('ui.cancel') }}</a>
        </form>
    </div>
</div>
@endsection
...
const selectedStation = {{ old('station_id', $user->station_id ?? 'null') }};
